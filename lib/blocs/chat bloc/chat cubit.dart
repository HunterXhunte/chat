...
import 'package:chat_try_3/models/message%20model.dart';
import 'package:chat_try_3/models/user%20model.dart';
        ...
        .listen((event) async {
          if (allMessages.isEmpty) {
            var messages = await firestore
                .collection("chats")
                .where("msgId",
                    whereIn: ["${registeredUser.id}${users[index].id}",
                      "${users[index].id}${registeredUser.id}"])
                .orderBy("time")
                .get();
            ...
            //   if (m.senderId == registeredUser.id ||
            //       m.senderId == users[index].id) {
